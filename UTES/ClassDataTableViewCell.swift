...
class ClassDataTableViewCell: UITableViewCell ,UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    var viewController: CalculateViewController!
    
    
    let classtypeList = ["","既修外国語（必修）", "初修外国語（必修）", "情報（必修）", "スポ身（必修）",
                         "初ゼミ文科（必修）","社会科学", "人文科学",
                         "基礎実験（自然科学）", "数理科学（自然科学）", "物理科学（自然科学）","生命科学（自然科学）",
                         "L（総合科目）", "A（総合科目）", "B（総合科目）", "C（総合科目）",
                         "D（総合科目）", "E（総合科目）", "F（総合科目）"]
    ...
    override func awakeFromNib() {
        ...
        classTimeInfoPickerView.selectRow(1, inComponent: 0, animated: true)
        classTimeInfoPickerView.selectRow(2, inComponent: 1, animated: true)
        
        let toolbar = UIToolbar(frame: CGRectMake(0, 0, 0, 35))
        let doneItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(ClassDataTableViewCell.done))
        let cancelItem = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(ClassDataTableViewCell.cancel))
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        classDataArray[textField.tag] = textField.text!
        viewController.classDataArray[self.tag] = classDataArray
        
        ud.set(viewController.classDataArray, forKey: "classArray")
        ud.synchronize()
        viewController.classDataTableView.reloadData()
